...
def repls(txt):
    ...
    txt = txt.replace("(`", "('")
    txt = txt.replace("`)", "')")
    txt = txt.replace(" = ss.TrainUpdate", " = ss.TrainUpdate.value")
    txt = txt.replace(" = ss.TestUpdate", " = ss.TestUpdate.value")
    txt = txt.replace(" ss.TrainUpdate >", " ss.TrainUpdate.value >")
    txt = txt.replace(" ss.TestUpdate >", " ss.TestUpdate.value >")
    return txt
...
inserts = [
    ...
    ("def Stopped(ss):", 10, ["""            ss.UpdateClassView()"""]),
    (
...
replaces = [
    ...
    (
        ...
        ["""        nv = netview.NetView()""", """        tv.AddTab(nv, "NetView")"""],
    ),
    (
        [
    ...
    (
        ...
        ["""        plt = eplot.Plot2D()""", """        tv.AddTab(plt, "RunPlot")"""],
    ),
    (
        [
            """        plt = *eplot.Plot2D(tv.AddNewTab(eplot.KiT_Plot2D, "TrnEpcPlot"))"""
